...
vector<double> r2dv, r2dvm, arv,arve, amodv, like_returnv, starlikev, starlikevm, like_returnvm, Xgcows,Ygcows,Rgcows,Zmax_gcows,rho_gcows, pOldv, pOldvm, Zmax_star, Zmax_starm, min_accelv,max_accelv,norm_posv, lnl_v, lnl_vm, addlnL_gcows, addlnL_m;
vector<int> gcows_vrows, gcows_vcols;

double density_normvgc, density_normvm, max_r, Rcut, innerCut, outerCut;
...
double star_likeZ(double z0modv, void* data)
{
  ...
  if (arve[iii] > 0.0)
    {
        ...
        //amodv[iii] = -1.0*GM/(sqrt(1.0 + (z0modv*z0modv/r2dv[iii]*r2dv[iii])))/(r2dv[iii]*r2dv[iii] + z0modv*z0modv);
  ...
  //cout << "Ln accel L: " << lnl_v[iii] << endl;
  lnl_v[iii] -= gmodv * log(sqrt(r2dv[iii]*r2dv[iii]+z0modv*z0modv)/brmodv);
  //cout << "a_mod for Gcow star " << amodv[iii] << endl;
  lnl_v[iii] += ((gmodv-almodv)/demodv) * log(1.0+pow((sqrt(r2dv[iii]*r2dv[iii]+z0modv*z0modv)/brmodv),demodv));
  //cout << "and with density stuff " << lnl_v[iii] << endl;
  lnl_v[iii] += addlnL_gcows[iii];
  //cout << "Additive: " << addlnL_gcows[iii] << endl;
  ...
  if(isinf(like_returnv[iii])==1)
    {
        ...
        //cout << " amod: " << amodv[iii] << " and a measured: " << arv[iii] << endl;
        //cout << "-(am - a)^2: " << -1.0*(arv[iii]-amodv[iii])*(arv[iii]-amodv[iii]) << endl;
        //cout << "2*error^2: " << 2.0*arve[iii]*arve[iii] << endl;
        //cout << "L pos 1: " <<((gmodv-almodv)/demodv) * log(1.0+pow((sqrt(r2dv[iii]*r2dv[iii]+z0modv*z0modv)/brmodv),demodv)) << endl;
       // cout << "L pos 2: " << -1.0*gmodv * log(sqrt(r2dv[iii]*r2dv[iii]+z0modv*z0modv)/brmodv) << endl;
        //cout << "ln L: " << lnl_v[iii] << endl;
        //cout << "Add ln L: "<< addlnL_gcows[iii] << endl;
    }
  //if(like_returnv[iii]==0.0)
    //{
      //  cout << "Got zero in summation for gcows stars, INFO:" << endl;
        //cout << "Star " << iii << " and R " << r2dv[iii] << " and Z " << z0modv << endl;
        //cout << " amod: " << amodv[iii] << " and a measured: " << arv[iii] << endl;
        //cout << "-(am - a)^2: " << -1.0*(arv[iii]-amodv[iii])*(arv[iii]-amodv[iii]) << endl;
        //cout << "2*error^2: " << 2.0*arve[iii]*arve[iii] << endl;
...
double star_likeZmaser(double z0modv, void* data)
{
  ...
  //cout << "Schodel density stuff " << lnl_vm[iii] << endl;
  //lnl_vm[iii] += addlnL_m[iii];
  //cout << "Schodel add lnL: " << addlnL_m[iii] << endl;
  ...
  if(like_returnvm[iii]==0.0){cout << "Got zero in summation for schodel stars" << endl;}
...
double star_lnLZ(double z0modv, void* data)
{
    int iii = *(int *)data;
    if (arve[iii] > 0.0)
    {
...
//double star_lnLZmaser(double z0modv, void* data)
//{
   // int iii = *(int *)data;
  //  lnl_vm[iii] = -1.0 * gmodv * log(sqrt(r2dvm[iii]*r2dvm[iii]+z0modv*z0modv)/brmodv);
//    lnl_vm[iii] += ((gmodv-almodv)/demodv) * log(1.0+pow((sqrt(r2dvm[iii]*r2dvm[iii]+z0modv*z0modv)/brmodv),demodv));
  ...
  ez_thread(threadnum,NThreads)
    {
      for(int i=threadnum; i<num_stars; i+=NThreads)
      {
          ...
          //addlnL_gcows[iii] = -1.0*log(0.5*(exp(star_lnLZ(0.0,&iii))+exp(star_lnLZ(Zmax_star[iii],&iii))));
          //if (addlnL_gcows[iii] == 0.0)
          //{
            //  addlnL_gcows[iii] = -1.0*star_lnLZ(0.0,&iii);
              //if (addlnL_gcows[iii] < (-1.0*star_lnLZ(Zmax_star[iii],&iii)))
              //{
         // addlnL_gcows[iii] = -1.0*star_lnLZ(Zmax_star[iii]/2.0,&iii);
          
          //if ((exp(star_lnLZ(0.0,&iii) + addlnL_gcows[iii]) == 0.0) | isinf(exp(star_lnLZ(0.0,&iii) + addlnL_gcows[iii])))
          //{
            //  cout << "Star " << iii << " got updated for add lnL" << endl;
             // addlnL_gcows[iii] = -1.0*star_lnLZ(0.0,&iii);
//              if ((exp(star_lnLZ(0.0,&iii) + addlnL_gcows[iii]) == 0.0) | isinf(exp(star_lnLZ(Zmax_star[iii],&iii) + addlnL_gcows[iii])))
          //}
          //if (iii == 97){addlnL_gcows[iii]=1620.83;}
          //if (iii == 47){addlnL_gcows[iii]=1538.89;}
              //}
          //}
          
          //if (isinf(log(star_likeZ(0.0,&iii)))==1)
          //{
            //  cout << "False alarm, log(L(z=0)) for Gcows = -inf, not using it" << endl;
              //addlnL_gcows[iii] = -2.0*log(star_likeZ(Zmax_star[iii],&iii))-0.7;
              //cout << "Gcows add lnL z=0: " << log(star_likeZ(0.0,&iii)) << endl;
              //cout << "Gcows add lnL z=max: " << log(star_likeZ(Zmax_star[iii],&iii)) << endl;
              //cout << "Gcows add lnL calc: " << addlnL_gcows[iii] << endl;
          //}
          //else
          //{
            //  addlnL_gcows[iii] = -1.0*log(star_likeZ(0.0,&iii)+star_likeZ(Zmax_star[iii],&iii))-0.7;
              //cout << "Gcows add lnL z=0: " << log(star_likeZ(0.0,&iii)) << endl;
              //cout << "Gcows add lnL z=max: " << log(star_likeZ(Zmax_star[iii],&iii)) << endl;
              //cout << "Gcows add lnL calc: " << addlnL_gcows[iii] << endl;
          //}
          //addlnL_gcows[iii] = -1.0*log(star_likeZ(0.0,&iii)+star_likeZ(Zmax_star[iii],&iii))-0.7;
          //if (isinf(addlnL_gcows[iii])==1){addlnL_gcows[iii] = -1.0*log(star_likeZ(Zmax_star[iii],&iii))-0.7;}
          //cout << "Gcows add lnL z=0: " << log(star_likeZ(0.0,&iii)) << endl;
          //cout << "Gcows add lnL z=max: " << log(star_likeZ(Zmax_star[iii],&iii)) << endl;
          //cout << "Gcows add lnL calc: " << addlnL_gcows[iii] << endl;
	  ...
	  //cout << "prob old " << pOldv[iii] << endl;
          starlikev[iii] /= (density_normvgc); // * norm_posv[iii]);
          starlikev[iii] = pOldv[iii] * (log(starlikev[iii]));// - addlnL_gcows[iii]);
      ...
      ez_thread(threadnum,NThreads)
        {
            ...
            {
                int iii = i;
                //cout << "Schodel add lnL z=0 " << log(star_likeZmaser(0.0,&iii)) << endl;
                //cout << "Schodel add lnL z=max " << log(star_likeZmaser(Zmax_starm[iii],&iii)) << endl;
                
                //addlnL_m[iii] = -1.0*log(0.5*(exp(star_lnLZmaser(0.0,&iii)) + exp(star_lnLZmaser(Zmax_starm[iii],&iii))));
                
                //if (addlnL_m[iii] == 0.0)
                //{
                //addlnL_m[iii] = -1.0*star_lnLZmaser(0.0,&iii)-0.7;
                //}
