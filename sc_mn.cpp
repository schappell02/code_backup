...
char root[100] = "/u/schappell/pmnOld/c_accel_16mag_";// root for output files
...
double density_int(double Rprime, double Zprime, void* data)
{
  ...
  return Rprime*pow((Rprime*Rprime + Zprime*Zprime),(gmod/-2.0))*pow((1.0+tmpvalue),((gmod-almod)/demod));
}
...
double star_likeZ(double z0mod, void* data)
{
  ...
  like_den *= pow((r2d[stardex]*r2d[stardex]+z0mod*z0mod),(gmod/-2.0));
  ...
  if (density_norm <= 0.0)
    {
      ...
      density_norm = gauss_legendre_2D_cube(100,density_int,NULL,0,max_R,0,use_maxZ);
...
int main(int argc, char *argv[])
{
  ...
  ifstream in("stars_mn_S0-17.dat",ios::in);
