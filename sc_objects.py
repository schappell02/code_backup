...
class Star(object):
	...
	self.zerr = rerr[2]
    def getRerr(self):
	return Vector([self.xerr, self.yerr, self.zerr])
	...
	self.vzerr = verr[2]
    def getVerr(self):
	return Vector([self.vxerr, self.vyerr, self.vzerr])
...
class Transform(object):
    ...
    def loadAbsolute(self):
        ...
        #self.sgra = [-0.003, -0.008]
        #self.sgraErr = [0.001, 0.003]
	...
	transTab = asciidata.open(root + align)

	self.numStars = transTab[1][idx]
	self.a = [transTab[3][idx], transTab[5][idx], transTab[7][idx]]
	self.b = [transTab[9][idx], transTab[11][idx], transTab[13][idx]]
	self.aerr = [transTab[4][idx], transTab[6][idx], transTab[8][idx]]
	self.berr = [transTab[10][idx], transTab[12][idx], transTab[14][idx]]

    def loadFromAlignTrans(self, root, idx=1):
	transTab = asciidata.open(root + '.trans')
    ...
    def loadFromAlign(self, root):
        if os.path.exists(root + '.sgra'):
            sgraTab = asciidata.open(root + '.sgra')
            self.sgra = [sgraTab[0][0], sgraTab[1][0]]
            self.sgraErr = [0.0, 0.0]
            self.angle = sgraTab[2][0]
            self.angleErr = sgraTab[3][0]
        else:
            self.sgra = [0.0, 0.0]
            self.sgraErr = [0.0, 0.0]
            self.angle = 0.0
	...
	scaleTab = asciidata.open(root + '.scale')
	self.scale = scaleTab[0][0]
	self.scaleErr = 0.0
