...
double use_maxZ, density_norm, density_norm_maser, density_gcows, density_schodel;
//for certain models the max Z needs to be smaller for integration to work
double density_normM, density_box, density_cyl, use_min_accel, blahDex;
...
double density_int2(double Rprime, double Zprime, void* data)
{
...
double density_intZ2(double Zprime, void *data)
{
...
double density_intZ2Q(double Zprime)
{
  double tmpvalue = pow((sqrt(Xprime*Xprime + Yprime*Yprime + Zprime*Zprime)/brmod),demod);
  return fabs(dxy*dxy*pow((Xprime*Xprime + Yprime*Yprime + Zprime*Zprime)/(brmod*brmod),(gmod/-2.0))*
	      pow((1.0+tmpvalue),((gmod-almod)/demod)));
...
double density_intZcyl2(double Zprime, void* data)
{
...
double density_intR2(double Rprime, void* data)
{
  ...
  Rcyl = Rprime * 1.0;
  return gauss_legendre(100,density_intZcyl2,NULL,min_Z,max_Z);
}
...
double density_sphere2(double r3dprime, void* data)
{
  //cout << "Density integration over R and z" << "\n";
...
double pos_int2(double ax, void* data){return exp(-1.0*(ar[stardex]-ax)*(ar[stardex]-ax)/(2.0*are[stardex]*are[stardex]));}

double star_likeZ2(double z0mod, void* data)
{
  //cout << z0mod << "\n";
  double like_pos;
  if ((ar[stardex]) < 0.0)
...
double star_likeZmaser2(double z0mod, void* data)
{
  //cout << z0mod << "\n";
...
vector<double> LogLike2(double *Cube)
{
  //cout << "in log like 2" << endl;
  ...
  if (fabs(remainder(situation,2)) > 0.0)
    {
      ...
      brmod = Cube[3] * (max_b - min_b) + min_b; //flat prior on r_break
      //brmod = exp((log(max_b)-log(min_b))*Cube[3] + log(min_b)); //log prior on r_break
      //brmod = sqrt(2.0) * sigma_b * boost::math::erf_inv(2.0*Cube[3] - 1.0) + meu_b; //gaussian prior on r_break

      if (situation == 3){cmod = Cube[4];}
      //rho0mod = Cube[5] * max_rho0;
      //rhmod = Cube[5] * max_rh;
    }
  //cout << "Defined param" << endl;
  if (nonRadial > 0)
    {
      ...
      for(int i=0; i<num_gcows; i++)
	{
	  ...
	  if ((Rprime < Rcut) & (Rprime >= 0.0))
	      ...
	      else
		{
	      ...
	      //quadrature::adaptive().relative_accuracy(1e-5).absolute_accuracy(1e-7)(density_intZ2Q,min_Z,max_Z,tmpDen,result_error);
	      double tmpDen = gauss_legendre(100,density_intZ2,NULL,min_Z,max_Z);
	      ...
	      if (tmpDen <= 0.0)
		{
		  ...
		  if (use_maxZ < max_Z)
		    {
		      tmpDen = gauss_legendre(100,density_intZ2,NULL,min_Z,use_maxZ);
	      ...
	      //cout << i << " max Z " << std::setprecision(20) << max_Z << " and density " << std::setprecision(20) << tmpDen << endl;
	      //tmpDen = 1e47;
	      density_norm += tmpDen;
	      density_gcows += tmpDen;
	      blahDex += 1.0;
  ...
  else
    {
      ...
      density_norm = gauss_legendre(100,density_intR2,NULL,0,Rcut);
      double density_hole = gauss_legendre(100,density_sphere2,NULL,0,rhmod);
  ...
  //cout << " " << "\n";
  //cout << "GCOWS norm: " << density_norm << endl;
  ...
  if(situation > 2)
    {
      ...
      double density_tmp = gauss_legendre(100,density_intR2,NULL,innerCut,outerCut);
      if (density_tmp <= 0.0)
	{
      ...
      density_schodel += (1.0 - cmod) * density_tmp;
  ...
  //cout <<"Gamma is "<<gmod<<", alpha is "<<almod<<", delta is "<<demod<<", and break r is "<<brmod<<"\n";
  //cout <<"Density norm = " << density_norm << endl;
  ...
  //cout << "Density norm calc" << endl;
  double norm_pos,min_Z;
  for(int i=0; i<num_stars; i++)
    {
      //cout << "in gcows stars loop" << endl;
      stardex = i;
      //cout << ar[i] << " " << are[i] << " " << pOld[i] << endl;
      //cout << "Begin starlike" << "\n";
      ...
      if ((ar[stardex]) < 0.0)
	{
	  ...
	  if(norm_pos==0.0){norm_pos=gauss_legendre(40,pos_int2,NULL,use_min_accel,max_accel[stardex]);}
      ...
      else
	{
      ...
      starlike = gauss_legendre(100,star_likeZ2,NULL,min_Z,max_Z);
      //cout << "INDEX " << stardex << endl;
      //cout << "starlike  " << starlike << endl;
      //cout << "norm pos " << norm_pos << endl;
      //cout << "Density norm" << density_norm << endl;
      //cout << "prob old " << pOld[i] << endl;
      double tmpvalue = pOld[i] * log(starlike * cmod / (norm_pos * density_norm));
      if(tmpvalue < -1e20){tmpvalue=0.0;}
      if(tmpvalue != tmpvalue){tmpvalue = 0.0;}
      if(isinf(tmpvalue)==1){tmpvalue = 0.0;}
      //cout << "Final " << tmpvalue << endl;
      //cout << "Current total " << total_lnL+tmpvalue << endl;
      //cout << " " << endl;
      total_lnL += tmpvalue;
      //cout << starlike << "\n";
  ...
  if (situation > 2)
	...
	{
	  //cout << "Cycle through schodel/maser stars" << endl;
	  maserdex = m;
	  //cout << ar[m] << " " << are[m] << " " << pOld[m] << " maser" << endl;
	  //cout << "Begin starlike" << "\n";
	  double max_Z = sqrt(max_r*max_r - r2dm[maserdex]*r2dm[maserdex]);
	  starlike = gauss_legendre(100,star_likeZmaser2,NULL,0,max_Z);
	  //cout << "INDEX " << maserdex << endl;
	  //cout << "starlike  " << starlike << endl;
	  //cout << "Density norm " << density_norm << endl;
	  //cout << "prob old " << pOldm[m] << endl;
	  double tmpvalue = pOldm[m] * log(starlike * (1.0 - cmod) / density_norm);
	  if(tmpvalue < -1e20){tmpvalue=0.0;}
	  if(tmpvalue != tmpvalue){tmpvalue = 0.0;}
	  if(isinf(tmpvalue)==1){tmpvalue = 0.0;}
	  //cout << "Final " << tmpvalue << endl;
  ...
  //lnew = total_lnL * 1.0;
  //cout << "Total ln L " << total_lnL << endl;
  //cout << gmod << " " << almod << " " << demod << " " << brmod << " " << cmod << endl;
  //cout << "Gamma = " << gmod << "\n";
  //cout << "Alpha = " << almod << "\n";
  //cout << "Delta = " << demod << "\n";
  //cout << "R_break = " << brmod << "\n";
  //cout << "Constant = " << cmod << "\n";
  //cout << "R_hole = " << rhmod << "\n";
  return {density_norm, density_gcows, density_schodel, blahDex, total_lnL};
}

void dumper2(int &nSamples, int &nlive, int &nPar, double **physLive, double **posterior, double **paramConstr, double &maxLogLike, double &logZ, double &logZerr, void *context)
{
