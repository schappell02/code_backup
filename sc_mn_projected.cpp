...
double max_b = 25.0;//2.0*cm_in_pc;
...
{
  return fabs(2.0*PI*Rprime*pow((Rprime/brmodv),gmodv)*
	      pow((1.0+pow((Rprime/brmodv),demodv)),((gmodv-almodv)/demodv)));
}
...
void LogLike(double *Cube, int &ndim, int &npars, double &lnew, void *context)
{
  ...
  if (nonRadial > 0)
      ...
      for(int i=0; i<num_gcows; i++)
	{
	  ...
	  if ((Rgcows[iii] < Rcut) & (Rgcows[iii] >= 0.0))
	    {
	      density_normv += plate_scale * plate_scale * dist * dist * pow((1.0+pow((Rgcows[iii]/brmodv),demodv)),((gmodv-almodv)/demodv))*pow((Rgcows[iii]/brmodv),gmodv) / (au_in_pc * au_in_pc);
	      //cout << "Density norm step " << density_normv << endl;
  ...
  //cout << " Density norm after gcows " << density_normv << endl;

  if (situation > 2)
    {
      double tmp = gauss_legendre(100,density_intR,NULL,innerCut,outerCut) * dist * dist / (au_in_pc * au_in_pc);
      if (tmp <= 0.0)
	{
  ...
  //cout << "Density norm after schodel " << density_normv << endl;
  ...
  for(int i=0; i<num_stars; i++)
    {
      ...
      double tmpValue = cmodv * pow((1.0+pow((r2dv[iii]/brmodv),demodv)),((gmodv-almodv)/demodv));
      tmpValue *= pow((r2dv[iii]/brmodv),gmodv) * dist * dist / (au_in_pc * au_in_pc * density_normv);
      ...
      tmpValue = pOldv[iii] * log(tmpValue);
      if(tmpValue < -1e20){tmpValue=0.0;}
      if(tmpValue != tmpValue){tmpValue = 0.0;}
      if(isinf(tmpValue)==1){tmpValue = 0.0;}
  ...
  //cout << "ln L " << total_lnLv << endl;
  
  if (situation > 2)
    {
      //      ez_thread(threadnum,NThreads)
      //{
      for(int i=0; i<num_maser; i++)
	{
	  int iii = i;
	  //    starlikevm[iii] = gauss_legendre(100,star_likeZmaser,&iii,0.0,Zmax_starm[iii]);
	      ...
	      double tmpValue = (1.0 - cmodv) * pow((1.0+pow((r2dvm[iii]/brmodv),demodv)),((gmodv-almodv)/demodv));
	      tmpValue *= pow((r2dvm[iii]/brmodv),gmodv) * dist * dist / (au_in_pc * au_in_pc * density_normv);
	      //mutex.lock();
	      tmpValue = pOldvm[iii] * log(tmpValue);
	      //tmpValue = pOldv[iii] * log(tmpValue);
	      if(tmpValue < -1e20){tmpValue=0.0;}
	      if(tmpValue != tmpValue){tmpValue = 0.0;}
	      if(isinf(tmpValue)==1){tmpValue = 0.0;}
  ...
  //cout << "ln L after schodel " << total_lnLv << endl;
  ...
  //cout << gmodv << " " << almodv << " " << demodv << " " << brmodv << " " << cmodv << endl;
  if (total_lnLv == 0.0){total_lnLv = -1e21;}
  cout << "total ln L " << total_lnLv << endl;
  lnew = total_lnLv * 1.0;
...
int main(int argc, char *argv[])
{
  ...
  innerCut = atof(argv[8]) * au_in_pc / dist;
  outerCut = atof(argv[9]) * au_in_pc / dist;
	  ...
	  while(!in.eof())
	    {
	      ...
	      tmpx = sqrt((tmp2-1500.0)*(tmp2-1500.0))*plate_scale;
	      tmpy = sqrt((tmp1-1500.0)*(tmp1-1500.0))*plate_scale;
