...
double max_b = 15.0*dist*cm_in_au;//2.0*cm_in_pc;
...
double max_Z, max_R, maxr, min_Z, max_rho0;
//double almod = 4.0;
//double demod = 4.0;
//double brmod = 0.5 * cm_in_pc; //hold alpha, delta, and r_break constant
double use_maxZ, density_norm, density_norm_ad, density_norm_maser;
...
double max_X,max_Y,min_X,min_Y,innerCut,outerCut,ZprimeM,Rcut,Xprime,Yprime,Rprime,result_error;

//double density_intZ_R(double Zprime, void *data)
//{
...
//  double result, error;
//  gsl_function gf_f;
//  gf_f.function = &density_intZ_R;
//  gsl_integration_qags(&gf_f, 0.0, max_Z, 0, 1e-7, 1000, w, &result, &error);
//  gsl_integration_workspace_free(w);
// return result
//}
  

double density_intZR(double Zprime)
{
...
double density_int(double Rprime)//, double Zprime)
{
  //cout << "Density integration over R and z" << "\n";
  // double tmpvalue = pow((sqrt(Rprime*Rprime + Zprime*Zprime)/brmod),demod);
  //return fabs(2.0*PI*Rprime*pow((Rprime*Rprime + Zprime*Zprime)/(brmod*brmod),(gmod/-2.0))*
  //      pow((1.0+tmpvalue),((gmod-almod)/demod)));
  double result;
  quadrature::adaptive().relative_accuracy(1e-5).absolute_accuracy(1e-7)(density_intZR,min_Z,max_Z,result,result_error);
  return result;
}

double density_intZ(double Zprime)
{
...
double density_sphere(double r3dprime)
{
  //cout << "Density integration over R and z" << "\n";
...
double pos_int(double ax){return exp(-1.0*(ar[stardex]-ax)*(ar[stardex]-ax)/(2.0*are[stardex]*are[stardex]));}

double star_likeZ(double z0mod)
{
  //cout << z0mod << "\n";
...
double star_likeZmaser(double z0mod)
{
  //cout << z0mod << "\n";
  ...
  double like_return = like_den / density_norm;//_maser;
...
void LogLike(double *Cube, int &ndim, int &npars, double &lnew, void *context)
{
  ...
  if (nonRadial > 0)
      ...
      for(int i=0; i<num_gcows; i++)
	{
	  ...
	  //{
	      //cout << "In column loop" << "\n";
	  double Rprime = sqrt((gcows_rows[i]-1500.0)*(gcows_rows[i]-1500.0)+(gcows_cols[i]-1500.0)*(gcows_cols[i]-1500.0))*plate_scale;
	      //cout << "R of star " << Rprime << " and in gcows " << gcowsField[row][col] << "\n";
	  Rprime *= dist * cm_in_au;
	  if (Rprime < Rcut)
	    {
	      if (Rprime >= rhmod)
		{
	      ...
	      else
		{
	      ...
	      double tmpDen;
	      quadrature::adaptive().relative_accuracy(1e-5).absolute_accuracy(1e-7)(density_intZ,min_Z,max_Z,tmpDen,result_error);
  ...
  else
    {
      ...
      //density_norm = gauss_legendre_2D_cube(40,density_int,NULL,0,Rcut,0,max_Z);
      quadrature::adaptive().relative_accuracy(1e-5).absolute_accuracy(1e-7)(density_int,0.0,Rcut,density_norm,result_error);
      //if (density_norm <= 0.0)
      //{
      //  use_maxZ = 5.0 * brmod * pow(fabs(gmod/almod),(1.0/demod));
      //  density_norm = gauss_legendre_2D_cube(100,density_int,NULL,0,Rcut,0,use_maxZ);
      //  cout << "Using 5 times Z_rho,max as int limits" << "\n";
      //  if (density_norm <= 0.0)
      //    {
      //      use_maxZ = 3.0 * brmod * pow(fabs(gmod/almod),(1.0/demod));
      ...
      double density_hole;
      quadrature::adaptive().relative_accuracy(1e-5).absolute_accuracy(1e-7)(density_sphere,0.0,rhmod,density_hole,result_error);
      density_norm -= density_hole;
    }
  density_norm *= cmod;
  // density_norm_ad *= cmod;
  //cout << " " << "\n";
  //cout << "Old GCOWS norm: " << density_norm << "\n";
  ...
  if(situation > 2)
    {
      ...
      //double density_tmp = gauss_legendre_2D_cube(40,density_int,NULL,innerCut,outerCut,0,max_Z);
      double density_tmp;
      quadrature::adaptive().relative_accuracy(1e-5).absolute_accuracy(1e-7)(density_int,innerCut,outerCut,density_tmp,result_error);
      //      if (density_tmp <= 0.0)
      //{
      //  use_maxZ = 5.0 * brmod * pow(fabs(gmod/almod),(1.0/demod));
      //  density_tmp = gauss_legendre_2D_cube(100,density_int,NULL,innerCut,outerCut,0,use_maxZ);
      //  cout << "Using 5 times Z_rho,max as int limits" << "\n";
      //  if (density_tmp <= 0.0)
      //    {
      //      use_maxZ = 3.0 * brmod * pow(fabs(gmod/almod),(1.0/demod));
      ...
      //	  if (density_tmp <= 0.0)
      //	    {
      //	      cout << "Still have a problem with density integration, this is with schodel area";
      //	      cout <<"\n";
      //	      cout <<"Gamma is "<<gmod<<", alpha is "<<almod<<", delta is "<<demod<<", and break r is "<<brmod;
      //	      cout <<"\n";
      //	    }
      ...
      //cout << "Total density norm = " << density_norm << "\n";
    }
  //cout <<"Gamma is "<<gmod<<", alpha is "<<almod<<", delta is "<<demod<<", and break r is "<<brmod<<"\n";
  ...
  for(int i=0; i<num_stars; i++)
	...
	{
	  double min_Z = sqrt(rhmod*rhmod - r2d[i]*r2d[i]);
	}
      //starlike = gauss_legendre(100,star_likeZ,NULL,min_Z,max_Z);
      quadrature::adaptive().relative_accuracy(1e-5).absolute_accuracy(1e-7)(star_likeZ,min_Z,max_Z,starlike,result_error);
      //cout << starlike << "\n";
  ...
  if (situation > 2)
      ...
      for(int m=0; m<num_maser; m++)
	{
	  ...
	  quadrature::adaptive().relative_accuracy(1e-5).absolute_accuracy(1e-7)(star_likeZmaser,0.0,max_Z,starlike,result_error);
	  //cout << starlike << "\n";
	  if (starlike == 0.0){starlike=1e-323;}
